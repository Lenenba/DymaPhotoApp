...
import { Credentails,User } from "../interfaces/user.interface";
...
tryFetchCurrentUserEffect = createEffect(
  () => this.actions$.pipe(
    ...
    switchMap(() => this.authService.fetchCurrentUser().pipe(
      map((user: User | null ) =>
...
tryInscriptionEffect = createEffect(
  ...
  switchMap(({ user }: { user : User }) =>
    this.authService.inscription(user).pipe(
      ...
      catchError(err =>
        of(AuthActions.inscriptionErrorAction(
...
tryConnexionEffect = createEffect(
  ...
  switchMap(({ credentials }: { credentials : Credentails }) =>
    this.authService.connexion(credentials).pipe(
      ...
      catchError(err =>
        of(AuthActions.connexionErrorAction(
          {error: err.error ? err.error : 'une erreur est survenue'}
        )
      ))
    ))
